...
Game::Game(int width, int height, std::string title, std::string location,
  ...
  sprites[2].setTextureRect(sf::IntRect(64, 0, 32, 32));  // wheat seeded
                                                          // texture
  ...
  sprites[10].setTextureRect(
      sf::IntRect(64, 32, 32, 32));  // bean seeded texture
  sprites[11].setTextureRect(
      sf::IntRect(64, 64, 32, 32));  // corn seeded texture

  sprites[12].setTextureRect(sf::IntRect(128, 0, 32, 64));  // corn fully
                                                            // texture
  sprites[12].setOrigin(0, 32);

  sprites[13].setTextureRect(sf::IntRect(128, 64, 32, 64));  // corn half
                                                             // texture
  sprites[13].setOrigin(0, 32);

  sprites[14].setTextureRect(sf::IntRect(160, 0, 32, 64));  // bean fully
                                                            // texture
  sprites[14].setOrigin(0, 32);

  sprites[15].setTextureRect(sf::IntRect(160, 64, 32, 64));  // bean half
                                                             // texture
  ...
  int borderSize = 4;
  toolbar =
      new Menu(2, sf::Vector2f(rectWidth, rectHeight),
               sf::Vector2i(320 - rectWidth / 2, 480 - rectHeight), borderSize);
  ...
  rectHeight = 150;
  borderSize = 4;
  seedbar = new Menu(3, sf::Vector2f(rectWidth, rectHeight),
                     sf::Vector2i(320 - rectWidth / 2, 480 - rectHeight - 65),
                     borderSize);
...
void Game::run() {
  while (win->isOpen()) {
      ...
      if (event.type == sf::Event::MouseButtonPressed) {
        ...
        if (event.mouseButton.button == sf::Mouse::Left) {
          ...
          if (toolbar->isInside(mouseX, mouseY)) {
            ...
            // if planting
            if (toolMode == 3 &&
                land[TilePosY][TilePosX]->getLandType() == "Farmland") {
              // std::cout << "farmland\n";
        ...
        if (event.mouseButton.button == sf::Mouse::Right) {
          int TilePosX = mouseX / (win->getSize().x / cols);
          int TilePosY = mouseY / (win->getSize().y / rows);
          std::cout << "Tool mode: " << toolMode << " Seedmode: " << seedMode
                    << " , landtype: "
    ...
    if (toolMode == 1) {
      ...
      scythe.setPosition(Mouse::getPosition(*win).x,
                         Mouse::getPosition(*win).y);
      // scythe.setPosition(mouseX,mouseY);
    } else {
      scythe.setPosition(0, 0);
    }
    win->draw(scythe);
    /* #endregion */
